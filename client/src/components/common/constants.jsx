import {
  FaUserTie,
  FaPlusCircle,
  FaListAlt,
  FaUsers,
  FaChartLine,
  FaBriefcase,
  FaBookmark,
  FaFileAlt,
  FaChartBar,
  FaHome,
  FaClipboardList,
  FaEnvelope,
  FaInfoCircle,
  FaBlog,
} from "react-icons/fa";

export const nav = [
  {
    text: "home",
    path: "/",
    icon: <FaHome />,
  },
  {
    flag: "expand",
    text: "Requirement",
    path: "/requirements",
    icon: <FaClipboardList />,
  },

  { text: "Market Trends", path: "requirements/market-trends", icon: <FaChartLine /> },

];

export const filterData = [
  {
    city: "Delhi",
    expert: "Full Stack Developer",
    expected_salary: "50000-60000",
  },
  {
    city: "Gurugram",
    expert: "Backend Developer",
    expected_salary: "45000-55000",
  },
  {
    city: "Noida",
    expert: "Frontend Developer",
    expected_salary: "40000-50000",
  },
  { city: "Pune", expert: "Data Scientist", expected_salary: "70000-80000" },
  { city: "Surat", expert: "ML Engineer", expected_salary: "65000-75000" },
  {
    city: "Bangalore",
    expert: "UI/UX Designer",
    expected_salary: "55000-65000",
  },
  { city: "Hyderabad", expert: "Engineer", expected_salary: "48000-58000" },
  {
    city: "Mumbai",
    expert: "Mobile App Developer",
    expected_salary: "60000-70000",
  },
  {
    city: "Chennai",
    expert: "Blockchain Developer",
    expected_salary: "80000-90000",
  },
  {
    city: "Kolkata",
    expert: "Cyber Security Specialist",
    expected_salary: "75000-85000",
  },
  { city: "Ahmedabad", expert: "QA Engineer", expected_salary: "40000-45000" },
];

export const navExpandRecruiter = [
  {
    text: "Talent Search",
    path: "requirements/hire-talent",
    icon: <FaUserTie />,
  },
  {
    text: "Add Job Opportunity",
    path: "requirements/add-opportunity",
    icon: <FaPlusCircle />,
  },
  {
    text: "My Job Listings",
    path: "requirements/listed-opportunity",
    icon: <FaListAlt />,
  },
  {
    text: "Talent Pool",
    path: "requirements/talent-pool",
    icon: <FaUsers />,
  },
  {
    text: "Market Trends",
    path: "requirements/market-trends",
    icon: <FaChartLine />,
  },
];

export const navExpandCAndidate = [
  {
    text: "Opportunities",
    path: "requirements/browse-opportunities",
    icon: <FaBriefcase />,
  },
  {
    text: "Bookmarked Jobs",
    path: "requirements/bookmarked-jobs",
    icon: <FaBookmark />,
  },
  {
    text: "Resume Builder",
    path: "requirements/resume-builder",
    icon: <FaFileAlt />,
  },
  {
    text: "Market Trends",
    path: "requirements/market-trends",
    icon: <FaChartBar />,
  },
];

export const featured = [
  {
    cover: "../images/hero/h1.png",
    name: "Enhance Productivity",
    total: "Access specialized skills from partnering companies.",
  },
  {
    cover: "../images/hero/h2.png",
    name: "Trustworthy Transactions",
    total: "Our platform ensures secure transactions.",
  },
  {
    cover: "../images/hero/h3.png",
    name: "Skill Exchange Platform",
    total: "Explore a diverse range of skills offered by partnering companies.",
  },
  {
    cover: "../images/hero/h4.png",
    name: " Flexible Agreements",
    total: "Tailor agreements to your specific needs, ensuring transparency.",
  },
  {
    cover: "../images/hero/h6.png",
    name: " Seamless Communication",
    total: "Stay connected with your partners.",
  },
  {
    cover: "../images/hero/h6.png",
    name: " Seamless Communication",
    total: "Stay connected with your partners.",
  },
];

export const services = [
  {
    cover: "../images/services/s1.png",
    name: "Skill Exchange Platform",
    total: "Connect with experts from various fields.",
  },
];

export const list = [
  {
    id: 1,
    cover: "../images/list/p-1.jpeg",
    name: "Infosys",
    tech: "JavaScript/React.js",
    category: "Required",
    tag: "Connect",
    type: "5 Experts",
    location: "Gurugram",
  },
  {
    id: 2,
    cover: "../images/list/p-2.jpeg",
    name: "Accenture",
    tech: "Python/Django Developer",
    category: "Hiring",
    tag: "Connect",
    type: "4 Experts",
    location: "Gurugram",
  },
  {
    id: 3,
    cover: "../images/list/p-3.jpeg",
    name: "JP Morgon",
    tech: "Java/Spring",
    category: "Required",
    tag: "Connect",
    type: "6 Experts",
    location: "Surat",
  },
  {
    id: 4,
    cover: "../images/list/p-4.jpeg",
    name: "Wingify",
    tech: "SQL",
    category: "Hiring",
    tag: "Connect",
    type: "3 Experts",
    location: "Pune",
  },
  {
    id: 5,
    cover: "../images/list/p-5.jpeg",
    name: "Walmart",
    tech: "React Developer",
    category: "Required",
    tag: "Connect",
    type: "1 Expert",
    location: "Noida",
  },
  {
    id: 6,
    cover: "../images/list/p-6.jpeg",
    name: "Deloitte",
    tech: "React Developer",
    category: "Hiring",
    tag: "Connect",
    type: "2 Experts",
    location: "Delhi",
  },
];
export const awards = [
  {
    icon: <i className="m-auto fa-solid fa-1"></i>,
    num: "Register",
    name: "Log-in or Sign-Up ",
  },
  {
    icon: <i className="m-auto fa-solid fa-2"></i>,
    num: "Search ",
    name: "Search Your Requirements to get and to provide experts",
  },
  {
    icon: <i className="m-auto fa-solid fa-3"></i>,
    num: "Match ",
    name: "Find experts who fullfill or matches Your Requirements ",
  },
  {
    icon: <i className="m-auto fa-solid fa-4"></i>,
    num: "Connect",
    name: "Connect with them and ignite innovation",
  },
];
export const location = [
  {
    id: 1,
    name: "Microsoft, India",
    Experts: "12 Experts",
    Apartments: "React Js",
    Offices: "07 Months",
    cover: "./images/location/company-1.jpeg",
  },
  {
    id: 2,
    name: "Goggle, United State",
    Experts: "20 Experts",
    Apartments: "Angular",
    Offices: "02 Months",
    cover: "./images/location/company-2.jpeg",
  },
  {
    id: 3,
    name: "Facebook, London",
    Experts: "12 Experts",
    Apartments: "React Js",
    Offices: "07 Months",
    cover: "./images/location/company-3.jpeg",
  },
  {
    id: 4,
    name: "IBM, United States",
    Experts: "12 Experts",
    Apartments: "React Js",
    Offices: "07 Months",
    cover: "./images/location/company-4.jpeg",
  },
  {
    id: 5,
    name: "Zoom, India",
    Experts: "12 Experts",
    Apartments: "Cyber Secrity ",
    Offices: "06 Months",
    cover: "./images/location/company-5.jpeg",
  },
  {
    id: 6,
    name: "Atlasian, USA",
    Experts: "03 Experts",
    Apartments: "React Js",
    Offices: "07 Months",
    cover: "./images/location/company-6.jpeg",
  },
  {
    id: 7,
    name: "Sony, USA",
    Experts: "03 Experts",
    Apartments: "React Js",
    Offices: "07 Months",
    cover: "./images/location/company-7.jpeg",
  },
  {
    id: 8,
    name: "Atlasian, USA",
    Experts: "03 Experts",
    Apartments: "React Js",
    Offices: "07 Months",
    cover: "./images/location/company-8.jpeg",
  },
];
export const team = [
  {
    list: "50",
    cover: "../images/customer/team-1.jpg",
    address: "Liverpool, Canada",
    name: "Sargam S. Singh",
    icon: [
      <i className="m-auto fa-brands fa-facebook-f"></i>,
      <i className="m-auto fa-brands fa-linkedin"></i>,
      <i className="m-auto fa-brands fa-twitter"></i>,
      <i className="m-auto fa-brands fa-instagram"></i>,
    ],
  },
  {
    list: "70",
    cover: "../images/customer/team-2.jpg",
    address: "Montreal, Canada",
    name: "Harijeet M. Siller",
    icon: [
      <i className="m-auto fa-brands fa-facebook-f"></i>,
      <i className="m-auto fa-brands fa-linkedin"></i>,
      <i className="m-auto fa-brands fa-twitter"></i>,
      <i className="m-auto fa-brands fa-instagram"></i>,
    ],
  },
  {
    list: "80",
    cover: "../images/customer/team-3.jpg",
    address: "Denever, USA",
    name: "Anna K. Young",
    icon: [
      <i className="m-auto fa-brands fa-facebook-f"></i>,
      <i className="m-auto fa-brands fa-linkedin"></i>,
      <i className="m-auto fa-brands fa-twitter"></i>,
      <i className="m-auto fa-brands fa-instagram"></i>,
    ],
  },
  {
    list: "51",
    cover: "../images/customer/team-4.jpg",
    address: "2272 Briarwood Drive",
    name: "P. Grimaldo",
    icon: [
      <i className="m-auto fa-brands fa-facebook-f"></i>,
      <i className="m-auto fa-brands fa-linkedin"></i>,
      <i className="m-auto fa-brands fa-twitter"></i>,
      <i className="m-auto fa-brands fa-instagram"></i>,
    ],
  },
  {
    list: "42",
    cover: "../images/customer/team-5.jpg",
    address: "2272 Briarwood Drive",
    name: "Q. Grimaldo",
    icon: [
      <i className="m-auto fa-brands fa-facebook-f"></i>,
      <i className="m-auto fa-brands fa-linkedin"></i>,
      <i className="m-auto fa-brands fa-twitter"></i>,
      <i className="m-auto fa-brands fa-instagram"></i>,
    ],
  },
  {
    list: "38",
    cover: "../images/customer/team-5.jpg",
    address: "Montreal, USA",
    name: "Adam K. Jollio",
    icon: [
      <i className="m-auto fa-brands fa-facebook-f"></i>,
      <i className="m-auto fa-brands fa-linkedin"></i>,
      <i className="m-auto fa-brands fa-twitter"></i>,
      <i className="m-auto fa-brands fa-instagram"></i>,
    ],
  },
];

export const price = [
  {
    plan: "Basic",
    id: 1,
    package_price: "29",
    ptext: "per user, per month",
    list: [
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "99.5% Uptime Guarantee",
      },
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "100GB Cloud Storage",
      },
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "Email Support",
      },
      {
        change: "color",
        icon: <i className="m-auto fa-solid fa-x"></i>,
        text: "Dedicated Account Manager",
      },
      {
        change: "color",
        icon: <i className="m-auto fa-solid fa-x"></i>,
        text: "Priority Support",
      },
    ],
  },
  {
    best: <i className="m-auto fa-solid fa-crown"></i>,
    plan: "Standard",
    id: 2,
    package_price: "49",
    ptext: "per user, per month",
    list: [
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "99.9% Uptime Guarantee",
      },
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "250GB Cloud Storage",
      },
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "Chat & Email Support",
      },
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "Dedicated Account Manager",
      },
      {
        change: "color",
        icon: <i className="m-auto fa-solid fa-x"></i>,
        text: "Priority Support",
      },
    ],
  },
  {
    plan: "Platinum",
    id: 3,
    package_price: "99",
    ptext: "per 3 users, per month",
    list: [
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "100% Uptime Guarantee",
      },
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "500GB Cloud Storage",
      },
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "24/7 Premium Support",
      },
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "Dedicated Account Manager",
      },
      {
        icon: <i className="m-auto fa-solid fa-check"></i>,
        text: "Priority Support & SLA",
      },
    ],
  },
];

export const footer = [
  {
    title: "LAYOUTS",
    text: [
      { id: "home", list: "Home", url: "/" },
      { id: "about", list: "About Us", url: "/about" },
      { id: "service", list: "Services", url: "/services" },
      { id: "contact", list: "Contact", url: "/contact" },
      { id: "blog", list: "Blog", url: "/blog" },
    ],
  },
  {
    title: "RESOURCES",
    text: [
      { id: "faqs", list: "FAQs", url: "/faqs" },
      { id: "contract", list: "Contract Management", url: "/contract" },
      { id: "support", list: "Employee Support", url: "/support" },
      { id: "guides", list: "Guides & Tips", url: "/guides" },
      { id: "partners", list: "Our Partners", url: "/partners" },
    ],
  },
  {
    title: "COMPANY",
    text: [
      { id: "mission", list: "Our Mission", url: "/mission" },
      { id: "careers", list: "Careers", url: "/careers" },
      { id: "privacy", list: "Privacy Policy", url: "/privacy" },
      { id: "terms", list: "Terms of Service", url: "/terms" },
      { id: "press", list: "Press Releases", url: "/press" },
    ],
  },
  {
    title: "CONNECT",
    text: [
      { id: "linkedin", list: "Follow Us on LinkedIn", url: "/linkedin" },
      { id: "newsletter", list: "Subscribe to Newsletter", url: "/newsletter" },
      { id: "twitter", list: "Follow Us on Twitter", url: "/twitter" },
      { id: "facebook", list: "Like Us on Facebook", url: "/facebook" },
    ],
  },
  {
    title: "LEGAL",
    text: [
      { id: "disclaimer", list: "Disclaimer", url: "/disclaimer" },
      { id: "trademark", list: "Trademark Information", url: "/trademark" },
      { id: "licensing", list: "Licensing", url: "/licensing" },
      { id: "cookie", list: "Cookie Policy", url: "/cookie" },
    ],
  },
  {
    title: "SUPPORT",
    text: [
      { id: "contact", list: "Contact Us", url: "/contact" },
      { id: "faq", list: "FAQ", url: "/faq" },
      { id: "shipping", list: "Shipping & Returns", url: "/shipping" },
      { id: "warranty", list: "Warranty Information", url: "/warranty" },
    ],
  },
];

export const dummyRecentCards = [
  {
    company_name: "Zepto",
    company_website: "zepto.com",
    email: "contact@zepto.com",
    ph_no: "1234567890",
    available_expert: ["Software Engineer, Product Manager"],
    from: {
      date: "2024-04-30T18:30:00.000Z",
    },
    to: {
      date: "2024-06-29T18:30:00.000Z",
    },
    desc_requirement:
      "Zepto is looking for software engineers and product managers to work on our innovative tech solutions.",
    address: "Bangalore, India",
    documents: "uploads\\1722892261133.pdf",
    cover_Img:
      "https://mir-s3-cdn-cf.behance.net/project_modules/max_1200/537bea105787531.5f80d653e74ef.jpg",
    Status: "required",
  },
  {
    company_name: "Techify Solutions",
    company_website: "techifysolutions.com",
    email: "hr@techifysolutions.com",
    ph_no: "9876543210",
    available_expert: ["Data Scientist, UX Designer"],
    from: {
      date: "2024-05-15T18:30:00.000Z",
    },
    to: {
      date: "2024-07-14T18:30:00.000Z",
    },
    desc_requirement:
      "Techify Solutions is seeking talented data scientists and UX designers to join our dynamic team.",
    address: "Pune, India",
    documents: "uploads\\techify_requirement.pdf",
    cover_Img:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQTetAD4MghaF8eTN5Ir6O9gbSNwU7iijHDlQ&s",
    Status: "required",
  },
  {
    company_name: "InnovateX",
    company_website: "innovatex.com",
    email: "careers@innovatex.com",
    ph_no: "1122334455",
    available_expert: ["DevOps Engineer, Cloud Architect"],
    from: {
      date: "2024-06-01T18:30:00.000Z",
    },
    to: {
      date: "2024-08-01T18:30:00.000Z",
    },
    desc_requirement:
      "InnovateX is looking for DevOps engineers and cloud architects to help build our next-gen infrastructure.",
    address: "Hyderabad, India",
    documents: "uploads\\innovatex_requirement.pdf",
    cover_Img:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS-pkVr6UceZGnF0TxdlxftkJuapQ4sOA2VVA&s",
    Status: "required",
  },
  {
    company_name: "BrightFuture",
    company_website: "brightfuture.com",
    email: "jobs@brightfuture.com",
    ph_no: "9988776655",
    available_expert: ["Frontend Developer, QA Engineer"],
    from: {
      date: "2024-07-10T18:30:00.000Z",
    },
    to: {
      date: "2024-09-09T18:30:00.000Z",
    },
    desc_requirement:
      "BrightFuture is in need of frontend developers and QA engineers to enhance our web and mobile applications.",
    address: "Mumbai, India",
    documents: "uploads\\brightfuture_requirement.pdf",
    cover_Img: "https://thegoan.net/uploads/news/big_37840_Untitled.png",
    Status: "required",
  },
  {
    company_name: "NextGen Tech",
    company_website: "nextgentech.com",
    email: "hiring@nextgentech.com",
    ph_no: "7766554433",
    available_expert: ["Backend Developer, Cybersecurity Specialist"],
    from: {
      date: "2024-08-05T18:30:00.000Z",
    },
    to: {
      date: "2024-10-04T18:30:00.000Z",
    },
    desc_requirement:
      "NextGen Tech is seeking backend developers and cybersecurity specialists to work on cutting-edge projects.",
    address: "Chennai, India",
    documents: "uploads\\nextgentech_requirement.pdf",
    cover_Img:
      "https://media.glassdoor.com/l/cc/20/60/1e/nextgen-technologies-inc-usa.jpg",
    Status: "required",
  },
  {
    company_name: "Visionary Labs",
    company_website: "visionarylabs.com",
    email: "talent@visionarylabs.com",
    ph_no: "3344556677",
    available_expert: ["AI/ML Engineer, Full Stack Developer"],
    from: {
      date: "2024-09-01T18:30:00.000Z",
    },
    to: {
      date: "2024-11-01T18:30:00.000Z",
    },
    desc_requirement:
      "Visionary Labs is on the lookout for AI/ML engineers and full stack developers to push the boundaries of innovation.",
    address: "Delhi, India",
    documents: "uploads\\visionarylabs_requirement.pdf",
    cover_Img:
      "https://www.shutterstock.com/image-photo/x-logo-san-francisco-on-260nw-2341306523.jpg",
    Status: "required",
  },
];

export const locations = [
  "Bangalore",
  "Hyderabad",
  "Mumbai",
  "Pune",
  "Chennai",
  "Delhi",
  "Kolkata",
  "Ahmedabad",
  "Noida",
  "Gurgaon",
  "Jaipur",
  "Coimbatore",
  "Chandigarh",
  "Lucknow",
  "Bhubaneswar",
];

export const skills = [
  "React",
  "JavaScript",
  "Node.js",
  "Python",
  "TypeScript",
  "Angular",
  "Java",
  "Spring Boot",
  "PHP",
  "Laravel",
  "C#",
  ".NET",
  "Flutter",
  "Django",
  "Kotlin",
  "Swift",
  "MySQL",
  "MongoDB",
  "AWS",
  "Azure",
  "Docker",
  "Kubernetes",
  "HTML",
  "CSS",
  "SASS",
  "GraphQL",
  "REST API",
  "Go",
  "Rust",
  "Ruby on Rails",
];

export const experiences = [
  "Junior Developer",
  "Mid-level Developer",
  "Senior Developer",
  "Lead Developer",
  "Software Engineer",
  "Frontend Developer",
  "Backend Developer",
  "Full Stack Developer",
  "DevOps Engineer",
  "Data Engineer",
  "Machine Learning Engineer",
  "AI Specialist",
  "UI/UX Designer",
  "Cloud Engineer",
  "QA Engineer",
  "Product Manager",
  "Project Manager",
  "Mobile App Developer",
];
